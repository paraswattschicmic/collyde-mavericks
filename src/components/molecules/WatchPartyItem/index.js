...
const WatchPartyItem = ({
    ...
    const getTimeString = () => {
        ...
        if (new Date(startTime) <= new Date() && moment(new Date(startTime)).add(contentLength, 'minutes') > new Date()) {
            ...
            let dateAsSelectedTimeZone = moment(new Date(startTime)).add(userTimeZoneOffset, 'minutes').utc().format('h:mm A');
    ...
    return (
        <View style={{ marginBottom: _scaleText(15).fontSize, paddingHorizontal: _scaleText(5).fontSize }}>
            <View>
                <Video
                    source={item.videoInfo && item.videoInfo.url ? { uri: item.videoInfo.url } : WATCH_PAGE_VIDEO_LIST[indexNum % 4]}
            ...
            <View style={styles.container}>
                <View style={{ flex: 1.37 }}>
                    {!!startTime && <View style={{ flexDirection: 'row' }}>
                        <Text style={styles.time}>{getTimeString()}</Text>
                        {/* <Text style={styles.time}>{now ? '' : ` ${timeFormat}`}</Text> */}
                    </View>}
                    {!!startTime && <Text style={[styles.date, { fontFamily: 'SFProText-Bold', lineHeight: _scaleText(14).lineHeight }]}>{getDateString()}</Text>}
                    {!!startTime && <Text style={[styles.date, { fontSize: _scaleText(14).fontSize, color: '#818596' }]}>{getMonthYearString()}</Text>}
                </View>
                ...
                <View style={{ flex: 5 }}>
                    ...
                    <Text style={{ fontFamily: 'SFProText-Regular', fontSize: _scaleText(17).fontSize, lineHeight: _scaleText(14).lineHeight }}>{leagueName} ON {platformName}</Text>
                    <Text style={{ fontFamily: 'SFProText-Regular', color: '#818596', fontWeight: '400', fontSize: _scaleText(14).fontSize }}>{(joined >= 0 && joined < 4) ? `Join The Party!` : `${joined} Joined`} </Text>
                    ...
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginTop: 10, marginLeft: 10 }}>
                        {item.defaultGifs && item.defaultGifs.length > 0
                            ...
                            item.defaultGifs.map((item, index) => {
                                ...
                                return <View key={index} style={styles.profileImgContainer}>
                                    <FastImage
                            ...
                            getJoinedUserImageUrls().map((item, index) => {
                                ...
                                return <View key={index} style={styles.profileImgContainer}>
                                    <FastImage
                                        ...
                                        source={item.showDefault ? item.pictureUrl : (item.showDefault == false && item.pictureUrl != '') ? { uri: item.pictureUrl } : DEFUALT_PROFILE_FACEBOOK}
