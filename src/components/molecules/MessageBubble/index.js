...
export class MessageBubble extends Component {
    ...
    _renderNickname = nickname => {
        return nickname ? <Text style={{ fontSize: _scaleText(18).fontSize, color: '#424950', paddingBottom: 4 }}>{nickname}</Text> : null;
    };
    ...
    _renderMessageItem = message => {
        ...
        if (message.messageType == 'user' || message.messageType == 'admin') {
            if (message.message && ((message.message).includes('http') || (message.message).includes('www.'))) {
                ...
                return <View style={{ flex: 1, flexWrap: 'wrap', width: '100%', minHeight: _scaleText(70).fontSize, }}>
                    <RNUrlPreview
                        titleNumberOfLines={1} descriptionNumberOfLines={2} containerStyle={{ backgroundColor: 'transparent', flexWrap: 'wrap', flex: 1 }} faviconStyle={{
                            height: '100%', width: _scaleText(80).fontSize, borderTopLeftRadius: _scaleText(10).fontSize, borderBottomLeftRadius: _scaleText(10).fontSize,
                        }} imageStyle={{
                            height: '100%', width: _scaleText(80).fontSize, borderTopLeftRadius: _scaleText(10).fontSize, borderBottomLeftRadius: _scaleText(10).fontSize,
                        }} imageProps={{ resizeMode: 'cover', overflow: "hidden", }} text={url, url} />
                </View>
            ...
            return <ImageItem localUriPath={this.props.localUriPath ? this.props.localUriPath.replace('http://', 'https://') : this.props.localUriPath} isUser={this.props.isUser} message={message.tempUri ? message.tempUri.replace('http://', 'https://') : message.url ? message.url.replace('http://', 'https://') : strAuthedUrl ? strAuthedUrl : ''} />;
    ...
    _hasOwnReaction = () => {
        ...
        if (this.props.message && this.props.message && this.props.message.reactions) {
            ...
            hasOwnReaction = reactionList && reactionList[0] && reactionList[0].userIds && reactionList[0].userIds.includes(this.props.userData._id) ? true : false
        }
    ...
    isHost = () => {
        return this.props.message._sender && this.props.message._sender.role && this.props.message._sender.role === TEXT_CONST.IS_GROUP_HOST ? true : false;
    }
    ...
    render() {
            ...
            >
                ...
                >
                    {this._hasOwnReaction() || this.props.isUser ? (
                        <View
                            style={{
                                flexDirection: this.props.isUser ? 'row-reverse' : 'row', height: '100%', marginBottom: this.props.isUser ? _scaleText(10).fontSize : _scaleText(0).fontSize
                            }}
                        >
                            <Image
                                // source={require('../../../assets/icons/active_trophyIcon.png')}
                                source={this._hasOwnReaction()
                                    ? require('../../../assets/icons/active_trophyIcon.png')
                                    : require('../../../assets/icons/trophyIcon.png')
                                }
                                style={styles.trophyIcon}
                            />
                            <Text style={this._hasOwnReaction() ? styles.textReactionsCount : styles.textGrayReactionsCount}>{this._reactionListCount()}</Text>
                        </View>
                    ) : (
                            <View
                                style={{
                                    flexDirection: 'row', height: '100%', marginBottom: this.props.isUser ? _scaleText(10).fontSize : _scaleText(0).fontSize
                                }}
                            >
                                <Image
        ...
        return (
            ...
            <TouchableOpacity onLongPress={this.onPressMenu}
                style={this.props.isUser ? styles.currentUserBubbleWrap : this.props.isAdmin ? styles.adminBubbleWrap : styles.otherUserBubbleWrap}
            >
                        ...
                        <View style={{ flexDirection: 'row' }}>
                            {this.props.isAdmin ?
                                ...
                                this.props.isUser ?
                                    ...
                                    :
                                    <View
                                        style={[_isImage(this.props) ? styles.otherUserBubbleImage : styles.otherUserBubble, this.props.message && this.props.message.message && (this.props.message.message.includes('http') || this.props.message.message.includes('www.')) ? { paddingVertical: 0, backgroundColor: COLLYDE_MESSAGE_BUBBLE_BACKGROUND_COLOR, borderWidth: 1, borderColor: '#d9d9d9', padding: 0, paddingLeft: 0 } : null]}
                                    >
